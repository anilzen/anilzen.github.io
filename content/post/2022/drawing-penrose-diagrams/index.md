...
The [Penrose diagram](https://en.wikipedia.org/wiki/Penrose_diagram) is a valuable tool in relativity to illustrate the global causal structure of spacetimes[^1]. Qualitative Penrose diagrams are usually sufficient to illustrate the essential causal relationships. However, sometimes you want a quantitative diagram.

I used various tools over the years to draw such diagrams. In this post, I show how to draw Penrose diagrams using the LaTeX package TikZ, which [is not a drawing program](https://en.wikipedia.org/wiki/PGF/TikZ) but a program that draws[^2]. We'll use Python to perform transformations when we hit the computational limitations of TikZ.
...
where $d\sigma^2=d\theta^2+\sin^2\theta d\varphi^2$, the standard metric on the unit sphere. Penrose diagrams are two-dimensional, so we essentially ignore the angular coordinates (each point except the origin represents a sphere). Light propagates along the null directions. The in- and outgoing spherical light rays propagate along $t+r$ and $t-r$. 

The core idea of Penrose diagrams are new coordinates, $T$ and $R$, which compactify these null directions:
$$ T+R = \frac{2}{\pi}\textcolor{DarkOrchid}{\arctan}(t+r),$$ 
$$ T-R = \frac{2}{\pi}\textcolor{DarkOrchid}{\arctan}(t-r). $$
The mapped range of the coordinates is $T\in(-1,1)$ and $R\in[0,1)$. Note that I'm not including the endpoints yet. The Penrose compactification procedure involves the conformal completion of the spacetime which adds the points at infinity after a regularizing rescaling. This detail is not relevant for the diagrams but is important for geometry.
...
This is all we need to make plots. 

I'll demonstrate plotting time surfaces (level sets of a time coordinate) because this is my main application. Say, you'd like to see how constant $t$ surfaces look like in the Penrose diagram. You might be tempted to solve the above relationships for a graph function $T(R;t)$ with constant $t$. I prefer to let the machine do the work and plot the surfaces parametrically. You can define the functions $T(t,r)$ and $R(t,r)$ in TikZ as follows:
```latex
...
```
Then, plot the lines parametrically with the following code. 
```latex
...
| | | 
| :---: | :---: |
| ![](figures/mink_arctan.png "[Compactification with arctan](tikz/mink/mink_arctan.tex)") | ![](figures/mink_tanh.png "[Compactification with tanh](tikz/mink/mink_tanh.tex)") |

I plotted two versions of the diagram to illustrate a coordinate-dependent feature. On both diagrams, the $t$ surfaces intersect at spatial infinity, $i^0$. On the left diagram, they are tangent to each other, while on the right diagram they are not. The intersection of the $t$ surfaces is an invariant feature, but their behavior near $i^0$ on the Penrose diagram depends on coordinates. The left diagram uses the tangens function for compactification. Many other functions map an infinite domain to a finite domain. For example, using the hyperbolic tangens instead, the mapping would look like this
$$ T+R = \frac{2}{\pi}\textcolor{DarkOrchid}{\tanh}(t+r),$$ 
$$ T-R = \frac{2}{\pi}\textcolor{DarkOrchid}{\tanh}(t-r), $$
which gives the right diagram. The $t$ surfaces are not tangent at spatial infinity anymore.

I draw Penrose diagrams typically to present the causal structure of hyperboloidal surfaces. Such surfaces behave like spacetime hyperboloids: they are spacelike everywhere, including null horizons. In the literature, spacetime hyperboloids are typically defined by the level sets of $\tau$ in the equation
$$ t^2 - r^2 = \tau^2. $$
...
However, these surfaces are not generally useful to study evolution in time because intersect at future null infinity. It is not immediately obvious that they do intersect at null infinity if you don't know to look for it. But on the Penrose diagram below, one immediately sees the intersection. 

| | | 
| :---: | :---: |
| ![](figures/mink_hypal_intersect.png "[Intersecting hyperbolic slicing](./tikz/mink/mink_hypal_intersect.tex)") | ![](figures/mink_hypal_foliation.png "[Smooth hyperboloidal foliation](tikz/mink/mink_hypal_foliation.tex)") |

A better option for a foliation of Minkowski spacetime, illustrated on the right panel above, is to fix one such hyperboloid, say, with unit radius, and shift it in time by $\tau$, like this
$$ (t-\tau)^2 - r^2 = 1 \implies \tau = t \pm \sqrt{1+r^2}. $$
Choosing the minus sign gives a future hyperboloidal foliation. You can see that the surfaces do not intersect, instead providing a smooth foliation of future null infinity.

The causal structure of the time surfaces may not be immediately clear from the formulas. For example, many people find it hard to understand that hyperboloidal surfaces are spacelike at null infinity or that standard time surfaces intersect at spatial infinity. While we can demonstrate such properties by calculations, it's much easier to illustrate them on a Penrose diagram.

#### Reading data into TikZ
You may have noticed that the evaluation of the lines in the plots takes a while. TikZ is not designed for complicated mathematical operations. Once you move away from Minkowski spacetime, the transformations become too complicated for TikZ. In the past, I used Mathematica for such tasks, but Mathematica comes with an expensive license, and we'll use open source tools.

To handle complicated mathematical transformations, we generate the data for the plots in Python, write them in a file, and plot them with TikZ. The Python code to generate the files is below.
...
On the TikZ side, we simply read these points from the respective files and plot them. To plot data points, I use the library `pgfplots`. The nodes must be drawn with respect to the axis of the plot, which is achieved with the `axis cs:` directive. The essential part of the code is below.
...
\end{axis}
```
There's a separate line for each time surface read from a file. This is not the most elegant solution, but it works. If you're up for it, you might prefer using a library such as [tikzplotlib](https://pypi.org/project/tikzplotlib/) to streamline this part. But we are ready to move on to
