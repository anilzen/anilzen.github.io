...
But the simplicity of black holes seems to contradict the second law of thermodynamics, which states that total entropy never decreases. You can drop a high-entropy object, say a hot teacup, into the black hole and reduce the universe's entropy.
...
Counterintuitively, the maximal entropy of a region scales with the area of its boundary, not its volume. The [holographic principle](https://en.wikipedia.org/wiki/Holographic_principle) takes the area-dependence of the Bekenstein-Hawking entropy and pushes it further, suggesting that the dynamics in a volume of space is encoded on the boundary of that space. In other words, the information content of any spacetime region is encoded on the surface area of that region.

Take a break and think about this for a moment. How can our three-dimensional world be encoded on a two-dimensional surface? What does it mean? Is the universe a [simulation](https://en.wikipedia.org/wiki/Simulation_hypothesis) running on the screen of an alien child? Are we trapped in [The Matrix](https://en.wikipedia.org/wiki/The_Matrix)? Are we [Livin' on the Edge](https://www.youtube.com/watch?v=7nqcL0mjMjw)?

![Living on the Edge](living_on_the_edge.jpg "Aerosmith: Livin' on the Edge (Video 1993)")

### AdS/CFT correspondence
The first realization of the holographic principle was the [AdS/CFT correspondence](https://en.wikipedia.org/wiki/AdS/CFT_correspondence). Conjectured by [Maldacena](https://en.wikipedia.org/wiki/Juan_Mart%C3%ADn_Maldacena) in 1997, the correspondence suggests an equivalence between a [string theory](https://en.wikipedia.org/wiki/String_theory) in curved anti-de Sitter spacetime (AdS) and a strongly coupled [conformal field theory](https://en.wikipedia.org/wiki/Conformal_field_theory) in a lower-dimensional flat spacetime. Aside from the details, the remarkable aspect of the conjecture is the duality between the bulk and the boundary of AdS.

Below is a visual image for the AdS$\_3$/CFT$\_2$ correspondence. The cylinder represents the three-dimensional AdS$\_3$ spacetime. On the boundary of the cylinder lives the 1+1 dimensional CFT$\_2$. Slices of constant time are [hyperbolic manifolds](https://en.wikipedia.org/wiki/Hyperbolic_manifold), represented by a [Poincaré disk](https://en.wikipedia.org/wiki/Poincar%C3%A9_disk_model).

![AdS3](AdS3.png "AdS is hyperbolic space with a time direction. [Wikimedia](https://commons.wikimedia.org/wiki/File:AdS3.svg)")

A CFT along the boundary represents the dynamics of a theory with gravity in the Poincaré disk. The boundary of space encodes the bulk.
...
An essential feature of AdS in accomodating the holographic principle is its negative curvature. Consider the AdS$\_3$ metric in static coordinates
$$ ds^2_{\rm{AdS}} = - \left(1+r^2\right) d\tau^2 + \textcolor{RedViolet}{\frac{1}{1+r^2} dr^2 + r^2 d\theta^2},$$
Slices of constant time are hyperbolic spaces. Hyperbolic geometry has much more space near its conformal boundary than Euclidean space. It seems that flat spacetime with its Eucledean time slices would not allow such a description
...
$$ \tau=t-\sqrt{1+r^2}.$$
In this hyperboloidal time, Minkowski metric gets hyperbolic spatial slices
...
The agreement of the geometry of hyperboloidal and AdS slices is suggestive. Calculations in the AdS/CFT literature performed on time slices should carry over to flat spacetime in hyperboloidal coordinates. One such example is the Ryu-Takayanagi conjecture.

### RT if you agree

Ryu and Takayanagi (RT) have [proposed](https://arxiv.org/abs/hep-th/0603001) a relation between geometry on AdS and entropy on CFT. 
...
The two subsystems $A$ and $B$ of CFT$\_2$ live on the boundary of AdS$\_3$. They are separated by a screen $\gamma_A$ required to be an extremal surface in the bulk AdS$\_3$. An extremal "surface" on a slice of AdS$\_3$ is a geodesic connecting the meeting points of the domains $A$ and $B$ on the bounding circle. The RT proposal states that the entanglement entropy of subsystem $A$ is proportional to the area of the screen $\gamma_A$:
...
The theory uses Bondi coordinates along null infinity to discuss asymptotic symmetries, and switches to a hyperboloidal slicing to discuss massive particles. The slices they use have been first proposed as a cosmological model by [Milne](https://en.wikipedia.org/wiki/Milne_model) in 1935. Dirac used the underlying hyperboloids in 1949 in his paper on [Forms of Relativistic Dynamics](https://journals.aps.org/rmp/abstract/10.1103/RevModPhys.21.392). Milne slices are level sets of
$$ \tau = t^2 - r^2.$$
...
Again, we recognize the representation of Minkowski spacetime as a stack of hyperbolic disk. But in this case, the metric coefficients depend on time $\tau$. The mean extrinsic curvature of Milne slices vanishes asymptotically in time as $K=3/\sqrt{\tau}$. 
...
Milne coordinates have one advantage that a hyperboloidal foliation of null infinity cannot provide. A massive particle in constant motion will asymptote to a point on the hyperbolic slices. I'm not sure how important this feature is. But Milne coordinates are also hyperboloidal. So in a sense, celestial holography is already closely related to hyperboloidal holography.
